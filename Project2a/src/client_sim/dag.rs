...
impl<T> DAG<T> where T : Node{
    ...
    pub fn find_biggest_path(&self) -> (f64,Vec<usize>){
        ...
        //number of (remaining) outgoing nodes at idx
        ...
        for node_idx in 0..self.vertices.len() { //Just initializing ref_count here
            ...
            if ref_count[node_idx] == 0 { //If a node has no references, add it to the queue
                cur_leaves.push(node_idx);
                node_max[node_idx] = self.vertices[node_idx].get_weight();
            }
        }

        //used to construct path at the end
        let mut max_idx = 0;
        let mut max_value = 0.0;

        loop {
            //While there's still leaf nodes in the queue
            ...
            if leaf_value > max_value {
                max_value = leaf_value;
                max_idx = leaf_idx;
            }

            //Next we start removing this leaf from the graph
            for parent_idx_ref in leaf_ref.get_incoming() {

                //Each node that is outgoing to this node (a parent in ECS stuff) loses one references,
                //and falls closer to becoming a leaf node itself
                ...
                //If the path offered by this leaf is better than what's written down as the best option, write this instead!
    ...
    //Even more warmup (wanted to have a reasonable swath of DAG functionality, honestly might use in future)
